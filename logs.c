...
#define MSG_MISSING_DATE_TIME "The date or time is missing" 
...
#define MSG_UBX_ID "Warning: unknown ID"
...
#define MSG_INTERNAL_ERROR "Error: an internal error ocurred"
...
typedef enum {TIME_UPDATED, DATE_UPDATED, MISSING_DATE_TIME, TKPT_GEN, TKPT_PRINT, NMEA_GP, NMEA_FIX, UBX_ID, UBX_LONG, UBX_TIME, UBX_DATE, INCORRECT_STATEMENT, INTERNAL_ERROR, CHECKSUM, IGNORED_STATEMENT, SUCCESSFUL_EXIT, READ_FILE, OPEN_FILE} log_t;

const char log_dictionary[][MAX_STR] = {MSG_TIME_UPDATED, MSG_DATE_UPDATED, MSG_MISSING_DATE_TIME, MSG_TKPT_GEN, MSG_TKPT_PRINT, MSG_NMEA_GP, MSG_NMEA_FIX, MSG_UBX_ID, MSG_UBX_LONG, MSG_UBX_TIME, MSG_UBX_DATE, MSG_INCORRECT_STATEMENT, MSG_INTERNAL_ERROR, MSG_CHECKSUM, MSG_IGNORED_STATEMENT, MSG_SUCCESSFUL_EXIT, MSG_READ_FILE, MSG_OPEN_FILE};
...
void log_print(FILE * f, log_t current_log) {
	...
	if(current_log == UBX_LONG || current_log == UBX_TIME || current_log == UBX_DATE || current_log == INCORRECT_STATEMENT || current_log == INTERNAL_ERROR || current_log == CHECKSUM)
